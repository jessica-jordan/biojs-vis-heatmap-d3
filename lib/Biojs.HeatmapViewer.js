 ...
 */
	HEATMAP: (function($) {
		...
		var draw_axis = function() {
			...
			svg.selectAll("x_axis").data(config.x_axis).enter()
                        .append("text")
                        .attr("class","x_axis_text")
                        .style("font-size", font_size)
                        .style("display","block")
                        .attr("transform", function(d,i){
                          return "translate(330,0)rotate(270)";
                        }) 
                        .text(function(d) {
				return d +" XXXXXX XXXXdasdasdasddweazdasdasd";
			}).attr("x", function(d, i) {
				return 5;
			}).attr("y", function(d, i) {
				return i *  config.dimensions.cell_width - 16 * config.dimensions.cell_width;
			});
		...
		var redraw_map = function(argument){
                  console.log("redraw_map");
                  var svg_group,
                      targetSvg;
                  if (d3.select('#zoom_heatmap_div_svg').node() !== null){
                    console.log("zoom div there");
                    targetSvg = d3.select('#zoom_heatmap_div_svg');
                  }
                  else {
                    targetSvg = d3.select('#main_heatmap_div_svg');
                  }
                  svg_group = targetSvg
                                .selectAll('.heatmapGroup');
                  console.log("svg_group");
                  console.log(svg_group);
                  var maxLabelLength = checkLabelLengths(svg_group);
                  targetSvg
                     .attr("height", config.dimensions.canvas_height + config.canvas_margin.top + config.canvas_margin.bottom + maxLabelLength);

                  svg_group
                     .attr("transform", "translate(" + config.canvas_margin.right + "," + parseInt(config.canvas_margin.top + maxLabelLength) + ")");

                   
                }
                var checkLabelLengths = function(elGroup){
                  console.log(elGroup);
                  var labels = elGroup
                                .selectAll('.x_axis_text');
                  console.log(labels);
                  if (labels.length > 0 && labels.node() !== null){
                    console.log("labels nodes: ");
                    console.log(labels.node());
                    
                    var lengths = labels[0].map(function(text){
                      console.log("lengths forEach text:");
                      console.log(text);
                    ...
                    console.log("my lengths: ");
                    console.log(lengths);
                    var maxLen = getMaxOfArray(lengths);
                    console.log(maxLen);
                    return maxLen;
                  }
                    return 30;
	...
	_draw: function() {
		...
		var mapInit = this.HEATMAP.init(this.viewer_config, this._origData, this._MAIN_HEAT_MAP_DIV).draw();
                // mapInit.redraw();
	...
	_calcHeatMap: function(rangeObj) {
        ...
        _getMaxDescriptionSize: function(textEls){
           var textElsWidths = textEls.foreach(function(el,i){
             return el.getBBox().width;
           });
           return Math.max(textElsWidths); 
        }
});
